...
binary_file read_binary_file(std::filesystem::path path)
{
	printf("Reading %s\n", path.string().c_str());
...
text_file read_text_file(const char *file_name)
{
	printf("Reading %s\n", file_name);
...
void read_from_dir(const char* dir_name, text_file** text_files, u32& n_text_files, const char* contains)
{
	...
	for (const auto& entry : std::filesystem::directory_iterator(dir_name))
	{
		...
		if (entry.is_regular_file() && entry.path().filename().string().find(contains) != std::string::npos)
		{
			(*text_files)[m]= read_text_file(entry.path().string().c_str());
			m++;
		}
	}
}
