...
statistics mean_statistics(f64* a, u32 n)
{
	...
	f64 mean = sum / n;
	f64 stddev = sum2 / n - mean * mean;

	return { mean , std::sqrt(std::max(0.,stddev)) , min, max};
}
...
void print_statistics(const experiment_records& records)
{
	for (u32 u{}; u < records.n_experiments; u++)
	{
		...
		auto per_sample= [](f64 ns, u64 n) { return ns / (f64)n;  };
		auto [mean, msqrt, min, max] = 
			mean_statistics<u64>(results.times, results.n_samples, (f64(*)(f64, u64))per_sample, info.count*info.iterations);
		...
		printf("\t%s: %f (%f)\n",info.iterations==1 ? "Bandwidth GB/s: " : "Samples  GSamples/s:", mean_bw, msqrt_bw);
		printf("\tMin/Max: [%f  %f]\n", min_bw, max_bw);
	}
}